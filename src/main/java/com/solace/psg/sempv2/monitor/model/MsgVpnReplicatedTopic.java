...
public class MsgVpnReplicatedTopic {
  ...
  **/
  @ApiModelProperty(value = "The replication mode for the Replicated Topic. The allowed values and their meaning are:  <pre> \"sync\" - Messages are acknowledged when replicated (spooled remotely). \"async\" - Messages are acknowledged when pending replication (spooled locally). </pre> ")
