...
package com.solace.psg.sempv2.apiclient;

import com.solace.psg.sempv2.auth.ApiKeyAuth;
import com.solace.psg.sempv2.auth.Authentication;
import com.solace.psg.sempv2.auth.AuthenticationType;
import com.solace.psg.sempv2.auth.BearerTokenAuth;
import com.solace.psg.sempv2.auth.HttpBasicAuth;
import com.solace.psg.sempv2.auth.OAuth;
...
public class ApiClient
{
	...
	private String basePath = "http://www.solace.com/SEMP/v2";
	...
	public ApiClient()
	{
		...
		authentications.put(HttpBasicAuth.AUTH_TYPE, new HttpBasicAuth());
		// Prevent the authentications from being modified.
		// authentications = Collections.unmodifiableMap(authentications);
	}

	/**
	 * Sets authentication Type for the ApiClient. The default set by the
	 * constructor is Basic.
	 * 
	 * @param authType
	 */
	public void setAuthentication(AuthenticationType authType)
	{
		authentications = new HashMap<String, Authentication>();
		switch (authType)
		{
		...
		case None:
			break;
		case BearerToken:
			authentications.put(BearerTokenAuth.AUTH_TYPE, new BearerTokenAuth());
			break;
		case OAuth1_0:
		case OAuth2_0:
			authentications.put(OAuth.AUTH_TYPE, new OAuth());
			break;
		case ApiKey:
			authentications.put(ApiKeyAuth.AUTH_TYPE, new ApiKeyAuth("", ""));
			break;
		default:
			throw new UnsupportedOperationException("Current authentication type is not supported: " + authType.name());
		}
	}
	...
	{
		authentications = new HashMap<String, Authentication>();
		authentications.put(auth.getAuthType(), auth);
	}
	 ...
	 *                 http://www.solace.com/SEMP/v2/action
	...
	/**
	 * Helper method to set access token for the first BearerToken authentication.
	 *
	...
	public void setBearerToken(String bearerToken)
	{
		for (Authentication auth : authentications.values())
		{
			if (auth instanceof BearerTokenAuth)
			{
				((BearerTokenAuth) auth).setToken(bearerToken);
				return;
			}
		}
		throw new RuntimeException("No BearerToken authentication configured!");
	}
